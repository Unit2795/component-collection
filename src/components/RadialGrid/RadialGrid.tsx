...
/**
 * Polar grid renderer with interactive connection drawing
...
const RadialGrid = () => {
	...
	const calculateCellPath = useCallback(
		( radialIndex: number, spokeIndex: number ) => {
			...
			const p1 = toCartesian( {
				radius: innerRadius,
				angle: startAngle
			} );
			const p2 = toCartesian( {
				radius: outerRadius,
				angle: startAngle
			} );
			const p3 = toCartesian( {
				radius: outerRadius,
				angle: endAngle
			} );
			const p4 = toCartesian( {
				radius: innerRadius,
				angle: endAngle
			} );
